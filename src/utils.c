...
void tensor_min_max(float *a, int d0, int d1, int d2, int d3, float *min, float *max)
{
    ...
    for(int i = 0; i < d0; ++i){
        for(int j = 0; j < d1; ++j){
            for(int m = 0; m < d2; ++m){
            	for(int n = 0; n < d3; ++n) {
					int index = i * (d1 * d2 * d3) + j * (d2 * d3) + m * d3 + n;
					fmin = (a[index] < fmin) ? a[index] : fmin;
					fmax = (a[index] > fmax) ? a[index] : fmax;
            	}
            }
        }
    }
...
void nudge(float real_min, float real_max, int quant_min, int quant_max, float *nudged_min, float *nudged_max, float *nudged_scale)
{
